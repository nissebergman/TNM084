...
void computeVertex(int nr)
{
    ...
    /* -- Using standard noise function -- */    
  //float pNoise = noise(p);

    /* -- Doing something with FBM -- */

    float pNoise = 0;

    // Properties
    int octaves = 2;
    float lacunarity = 8.5;
    float gain = 1.4;
    ...
    /* -- Calculating normals -- */
    // För varje vertex p, räkna ut tre vektorer
    // baserat på normalen (??)
    // Ta små steg (0.1f ? längs dessa för att få 3 punkter runt vertexen)
    // Använd dessa punkter m triangle method för att få normal

    /* ------------------------- */

    // Normalizing p to get cube-sphere
    p = normalize(p) + (pNoise);
